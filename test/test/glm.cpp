...
{
	mat4 R = (rotation3D(vec3(1.0, 0.0, 0.0), rotate[0]));
	for (unsigned int i = 1; i <= model->numvertices; i += 1) {
		vec3 temp(model->vertices->at(3 * i + 0), model->vertices->at(3 * i + 1), model->vertices->at(3 * i + 2));
		temp = R * temp;
		model->vertices->at(3 * i + 0) = temp[0];
	...
	R = (rotation3D(vec3(0.0, 1.0, 0.0), rotate[1]));
	for (unsigned int i = 1; i <= model->numvertices; i += 1) {
		vec3 temp(model->vertices->at(3 * i + 0), model->vertices->at(3 * i + 1), model->vertices->at(3 * i + 2));
		temp = R * temp;
		model->vertices->at(3 * i + 0) = temp[0];
	...
	R = (rotation3D(vec3(0.0, 0.0, 1.0), rotate[2]));
	for (unsigned int i = 1; i <= model->numvertices; i += 1) {
		vec3 temp(model->vertices->at(3 * i + 0), model->vertices->at(3 * i + 1), model->vertices->at(3 * i + 2));
		temp = R * temp;
		model->vertices->at(3 * i + 0) = temp[0];
...
{
	mat4 T = translation3D(translate);
...
glmWriteOBJ_EXP(GLMmodel* model, char* filename, std::vector<simple_material> materials, std::string &materials_filename, int mode)
{
	...
	for (unsigned int i = 0; i < model->numtriangles; i += 1) {
		if (mode == COLORFUL){
			if (model->triangles->at(i).material_id != -1)
				fprintf(file, "usemtl %s\n", materials.at(model->triangles->at(i).material_id).name.c_str());
			else
...
void output_material(std::vector<simple_material> materials, std::string &filename)
{
	...
	fprintf(file, "newmtl initialShadingGroup\n");
	fprintf(file, "illum 4\n");
	fprintf(file, "Kd 0.50 0.50 0.50\n");
